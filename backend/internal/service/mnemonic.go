...
func computeW5Address(publicKey ed25519.PublicKey) string {
	walletCodeB64 := ""
	codeBytes, _ := base64.StdEncoding.DecodeString(walletCodeB64)

	codeHash := sha256.Sum256(codeBytes)
	pubHash := sha256.Sum256(publicKey)

	var stateData [64]byte
	copy(stateData[:32], codeHash[:])
	copy(stateData[32:], pubHash[:])
	stateHash := sha256.Sum256(stateData[:])

	rawAddr := hex.EncodeToString(stateHash[:])
	return "0:" + rawAddr
}

func GenerateTONMnemonic() []string {
	for {
		words := make([]string, 24)
		for i := range words {
			n, _ := rand.Int(rand.Reader, big.NewInt(int64(len(tonWords))))
			words[i] = tonWords[n.Int64()]
		}

		entropy := hmacSha512([]byte(strings.Join(words, " ")), []byte(""))

		if isBasicSeed(entropy) {
			return words
		}
	}
}

func hmacSha512(phrase, password []byte) []byte {
	mac := hmac.New(sha512.New, phrase)
	mac.Write(password)
	return mac.Sum(nil)
}

func isBasicSeed(entropy []byte) bool {
	salt := []byte("TON seed version")
	seed := pbkdf2.Key(entropy, salt, 390, 64, sha512.New)
	return seed[0] == 0
}
